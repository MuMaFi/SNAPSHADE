...
function drawHud(){
  ...
  hudX.fillStyle = HUDS.stamina > 0.2 ? 'rgba(206,196,156,0.75)' : 'rgba(206,120,80,0.8)';
  hudX.fillRect(bx, sy, bw*clamp(HUDS.stamina,0,1), 4);
...
bNv.addEventListener('click', e => { e.preventDefault(); toggleNv(); });
bUse.addEventListener('click', e => { e.preventDefault(); IN.use = true; });
bMenu.addEventListener('click', e => { e.preventDefault(); pauseGame(); });
...
function deathStep(dt){
  ...
  // Blickrichtung zur Gestalt: für die Kamera gilt atan2(-dx,-dz).
  // Mit dem früheren +PI drehte sie sich im Moment des Todes von ihr weg.
  const want = Math.atan2(-dx, -dz);
  let d = want - S.yaw;
  while(d >  Math.PI) d -= Math.PI*2;
  while(d < -Math.PI) d += Math.PI*2;
  S.yaw += d * Math.min(dt*7, 1);
  S.pitch += (0.16 - S.pitch) * Math.min(dt*4, 1);
  camera.position.set(S.pos.x, CFG.eye - Math.min(S.endT*0.5, 0.5) + Math.sin(S.endT*33)*0.05, S.pos.z);
  camera.rotation.set(S.pitch, S.yaw, Math.sin(S.endT*11)*0.06);
  camera.updateProjectionMatrix();
  MON.group.visible = true;
  MON.group.position.set(lerp(MON.group.position.x, S.pos.x, Math.min(dt*2,1)), 0,
                         lerp(MON.group.position.z, S.pos.z, Math.min(dt*2,1)));
  updateLights(dt, S.t, 1);
  S.glitch = 1;
  postMat.uniforms.uStatic.value = S.endT > 0.35 ? Math.min((S.endT-0.35)*3, 1) : 0;
  if(S.endT > 1.5) postMat.uniforms.uFade.value = Math.max(0, 1-(S.endT-1.5)*1.6);
  if(S.endT > 2.4 && scEnd.classList.contains('hidden')){
